...
describe("buildOrderLedger — item entries", () => {
  ...
  it("sets cancelled: true when canceled_quantity equals quantity, false otherwise", () => {
    ...
    expect(entries[0].partiallyCancelled).toBe(false);
    expect(entries[1].cancelled).toBe(false);
    expect(entries[1].partiallyCancelled).toBe(false);
  });

  it("sets partiallyCancelled: true (and cancelled: false) when canceled_quantity is between 0 and quantity, exposing canceledQuantity and cancellations", () => {
    const entries = buildOrderLedger({
      forms: [makeForm({ discount_in_cents: 0, items: [makePartiallyCancelledItem()] })]
    });
    expect(entries[0].cancelled).toBe(false);
    expect(entries[0].partiallyCancelled).toBe(true);
    expect(entries[0].canceledQuantity).toBe(basePartiallyCancelledItem.canceled_quantity);
    expect(entries[0].cancellations).toBe(basePartiallyCancelledItem.cancellations);
  });
