...
import AdditionalInputListCore from "../formik-inputs/additional-input/additional-input-list-core";
import showConfirmDialog from "../showConfirmDialog";

// Mocks
jest.mock("../showConfirmDialog", () => jest.fn());

function MockAdditionalInputComponent({
  item,
  itemIdx,
  onAdd,
  onDelete,
...
const renderWithFormik = (props, initialValues = { meta_fields: [] }) =>
  render(
    <Formik initialValues={initialValues} onSubmit={jest.fn()}>
      <Form>
        <AdditionalInputListCore {...props} />
      </Form>
    </Formik>
  );

describe("AdditionalInputListCore", () => {
  ...
  const defaultProps = {
    ...
    entityId: 1,
    AdditionalInputComponent: MockAdditionalInputComponent
  };
  ...
  describe("handleAddItem", () => {
    test("adds a new empty meta field when onAdd is called", async () => {
      const TestWrapper = () => {
        ...
        return (
          <>
            <AdditionalInputListCore {...defaultProps} />
    ...
    test("new meta field starts with empty values when an existing field already has values", async () => {
      ...
      const TestWrapper = () => {
        ...
        return <AdditionalInputListCore {...defaultProps} />;
      };
  ...
  describe("handleRemove", () => {
    ...
    test("calls API and removes from UI when item has id", async () => {
      ...
      const TestWrapper = ({ onDelete }) => {
        ...
        return (
          <>
            <AdditionalInputListCore {...defaultProps} onDelete={onDelete} />
            <div data-testid="field-count">{values.meta_fields.length}</div>
          </>
        );
      };
